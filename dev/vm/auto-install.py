...
def main() -> int:
    ...
    try:
        ...
        # Confirm share
        ser.cmd("test -f /mnt/proteus/dev/vm/guest-install.sh && echo SHARE_OK", wait=["SHARE_OK", "@archiso"])
        log("Running guest-install.sh (this takes several minutes)…")
        # Marker must not appear in the typed command echo — drain after send.
        ser.drain(0.5)
        ser.send(
            "bash /mnt/proteus/dev/vm/guest-install.sh 2>&1 | tee /tmp/guest-install.log; "
            "printf '\\n%s\\n' \"PROTEUS_GI_RC:$?\"\n"
        )
