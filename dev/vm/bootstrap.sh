...
#
# Prerequisites:
#   ./dev/vm/run.sh              # guest booted, sshd up
#   Base Arch already installed (./dev/vm/run.sh install + guest-install, or manual)
#   SSH key auth for PROTEUS_GUEST_USER (password prompts are not used here)
#
# Usage:
#   ./dev/vm/bootstrap.sh
#   PROTEUS_GUEST_USER=andrew ./dev/vm/bootstrap.sh
set -euo pipefail
...
echo "dev/vm/bootstrap: waiting for SSH ${USER}@${HOST}:${PORT} (publickey) …"
ready=0
...
if [[ "${ready}" -ne 1 ]]; then
  echo "dev/vm/bootstrap: FAIL SSH unreachable (BatchMode/publickey only)" >&2
...
echo "dev/vm/bootstrap: ensuring /mnt/proteus …"
...
echo "dev/vm/bootstrap: running overlay (sudo) …"
...
echo
echo "dev/vm/bootstrap: OK — reboot guest, then on host:"
echo "  ./dev/vm/run.sh snapshot hyprland-base"
echo "  PROTEUS_GUEST=1 ./dev/smoke-all.sh"
