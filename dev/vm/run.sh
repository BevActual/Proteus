...
#
# Usage:
#   ./dev/vm/run.sh install          # Arch ISO + disk (first install)
#   ./dev/vm/run.sh                  # disk only (post-install)
#   ./dev/vm/run.sh snapshot <name>  # qcow2 internal snapshot
#   ./dev/vm/run.sh restore <name>   # apply snapshot
#   ./dev/vm/run.sh snapshots        # list snapshots
#
...
#                                   Gamescope console session. Prereqs: dev/vm/README.md §VFIO
...
Create it with:  ./dev/vm/create-disk.sh"
}

case "${MODE}" in
  snapshot)
    [[ -n "${SNAP_NAME}" ]] || die "Usage: ./dev/vm/run.sh snapshot <name>"
    ensure_disk
    ...
    ;;
  restore)
    [[ -n "${SNAP_NAME}" ]] || die "Usage: ./dev/vm/run.sh restore <name>"
    ensure_disk
...
Usage: ./dev/vm/run.sh [install|run|snapshot|restore|snapshots]"
    ;;
esac
...
if [[ -n "${VFIO_SPEC}" ]]; then
  ...
  for _addr in "${_vfio_addrs[@]}"; do
    ...
    if [[ "${_drv}" != "vfio-pci" ]]; then
      ...
      echo "proteus-vm:      see dev/vm/README.md §VFIO (IOMMU + vfio-pci bind)" >&2
    fi
...
Download it with:  ./dev/vm/download-iso.sh"
