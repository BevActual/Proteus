...
#   bash /mnt/proteus/dev/dogfood/dogfood-console.sh --launch browser
#   bash /mnt/proteus/dev/dogfood/dogfood-console.sh --launch retroarch
#   bash /mnt/proteus/dev/dogfood/dogfood-console.sh --restore   # flip back to desktop
# Host: ssh … 'bash /mnt/proteus/dev/dogfood/dogfood-console.sh'
set -euo pipefail
...
if d.get("pad") is False:
  print("dogfood-console: hint pad — host: PROTEUS_VM_PAD=auto ./dev/vm/run.sh (keyboard stand-ins OK without pad)")
if d.get("isVm") and not d.get("gamescopeUsable"):
  print("dogfood-console: hint Gamescope — VirGL has no hardware Vulkan; interim kiosk (expected). GPU passthrough: dev/vm/README.md §VFIO")
' 2>/dev/null || true
