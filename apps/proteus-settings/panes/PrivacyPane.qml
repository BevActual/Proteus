...
ColumnLayout {
  ...
  ColumnLayout {
    ...
    Text {
      ...
      text: "App permissions gate adaptive apps, Flatpak overrides, portal PermissionStore (mic/camera/screen), and best-effort capture enforce (Deny + Ask mute/destroy for mic/camera/screen). Ask prompts at launch and mid-session (mic/camera/screen). Screen: portal Session.Close best-effort + PipeWire destroy fallback (attribution still imperfect). Not a full OS sandbox — fail-closed until Permissions.ready (privacy-gated apps wait)."
      color: Theme.textDim
      font.family: Theme.fontFamily
      font.pixelSize: 13
