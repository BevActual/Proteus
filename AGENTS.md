...
- **`smoke`** — host suite: `./scripts/smoke-all.sh` (layout · widget-layout-resolve · ipc-contract · config-schema · config-roundtrip · app-manifest · chrome-tokens · software-reliability · power-logind · accounts · lock-pin · permissions · desktop · spaces · focus · control-center · beacon · audio-mix-serve · hw-probe · install · session · posture-hard · console · host · qs-version; guest `qs-guest` + `software-guest` if SSH `:2222` or `PROTEUS_GUEST=1`)
