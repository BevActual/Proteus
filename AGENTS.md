...
- **`smoke`** — host suite: `./scripts/smoke-all.sh` (layout · widget-layout-resolve · ipc-contract · config-schema · config-roundtrip · app-manifest · chrome-tokens · software-reliability · power-logind · accounts · audio-mix-serve · hw-probe · install · session · qs-version; guest `qs-guest` + `software-guest` if SSH `:2222` or `PROTEUS_GUEST=1`)
