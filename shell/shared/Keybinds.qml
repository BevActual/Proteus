...
Singleton {
  ...
  readonly property var catalog: [
    ...
    },
    {
      ...
      key: "H",
      dispatcher: "exec",
      arg: "proteus-posture host --chrome",
      postures: ["desktop"]
    },
    {
      id: "console-nav",
  ...
  // Empty postures = all focus postures. Workspaces category → desktop only.
  function entryAllowedOnPosture(e, posture) {
    const p = String(posture || "desktop")
    const list = e && e.postures ? e.postures : null
    if (list && list.length) {
    ...
    if (e && e.category === "Workspaces")
      return p === "desktop"
    return true
  }

  function confText() {
    const posture = root.activePostureId()
    let out = "# Generated by Proteus Settings — Keyboard\n"
    out += "# Edit here or in Settings → Keyboard. Reloaded via hyprctl.\n"
    out += "# Filtered for posture: " + posture + "\n\n"
    let cat = ""
    for (let i = 0; i < catalog.length; i++) {
      const e = catalog[i]
      if (!root.entryAllowedOnPosture(e, posture))
        continue
      if (e.category !== cat) {
        cat = e.category
        out += "# " + cat + "\n"
    ...
    if (posture === "desktop" && typeof SpacesSpecials !== "undefined" && SpacesSpecials) {
    ...
    if (posture === "desktop" || posture === "host") {
      out += "# Mouse\n"
