#!/usr/bin/env bash
# vm/lib.sh — shared paths for Proteus QEMU harness
# Artifacts live outside the repo (modular cache). Source from other vm/*.sh:
...
proteus_vm_migrate_legacy() {
  ...
  for d in iso disks boot vars; do
    ...
    if [[ -z "$(find "${legacy}" -mindepth 1 ! -name '.gitkeep' -print -quit)" ]]; then
      rm -rf "${legacy}"
      echo "proteus-vm: removed empty legacy vm/${d}/"
    fi
  done
}
