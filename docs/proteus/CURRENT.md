...
| **Beacon** — system search (`Super+Space` / `Super+D`) | `partial` — iced **mode strip** Apps · Settings · Windows · Files (**Ctrl+1–4** + segmented control); Apps (desktop + clipboard + calc) · Settings catalog · running Windows · file index (`beacon-file-index.py`); empty-query **Places** + **Recents** in Files; **Clipboard** thin (`cliphist` → Enter decode\|`wl-copy`, optional `wtype`); **Calc** thin. Ghostty → `proteus-terminal`. Escape: `proteus-clipboard`. **Out / rebuild:** unit convert, image clipboard preview, Privacy grant search, QML-era Pinned richness |
| Dock (pins, layouts, running dots) | `shipped` — iced dock on **Top** with `exclusive_zone` from rest strip (`dock_strip_h`); Facts `dockLayout` (`center`\|`span`\|`left`\|`right`), `dockRounding`, `dockIconSize`, `dockEnabled`, `dockAutoHide` (slide + edge peek); **no magnify** — per-icon hover scale + stronger launch bounce; menu bar `barHeight` / `barRounding` / `barAutoHide`; running mute disc / focused pill / **1–3 multi-window dots**; divider pins ‖ transients; bottom-dock dwell preview (~350ms grim thumbs; vertical preview Out). **Long-press edit** (~450ms): reorder pinned cells (Beacon fixed at 0) + **(−) remove pin** + **drag-off strip** (drop below shelf to remove) + Done → `dockPins` persist. **Right-click glass menu** Keep (transient → pin) / Remove (unpin) → `dockPins` immediate persist. Settings → Desktop → Dock & menu bar. **Out:** `dockMonitor` / `barMonitor`; Win11 centered-on-span icons; vertical dwell previews |
...
+ row peaks live while Mixer open In; channel reorder ↑↓ + free-pointer grip drag In) · Notifications · Users · Privacy (activity +
...
Facts) · Accounts (multi-seat glances thin) · Displays. Holdouts: Tailscale deep ACL,
Headscale ACL visual graph.
([STACK.md](./STACK.md) §6).
...
| Desktop → Gaps / Borders / Motion / Dock & menu bar / Spaces / Default apps / Focus / Control Center / Beacon | `shipped` (thin iced) — Gaps live via `proteus-settings-apply`; Spaces `workspaceMode` Fact; **Default apps** picker (`proteus-defaults.py`); **Focus** enable + active picker + **profile CRUD** (add/rename/delete · keywordAllow/Deny · **allowedApps** CSV · **schedule** `{enabled,days,start,end,rrule?}` + **shell auto-apply** edge-triggered · thin **RRULE** `FREQ=WEEKLY;BYDAY=…` optional `UNTIL` · **Beacon/Dock launch enforce** when Focus on: non-empty allowedApps whitelist · keywordDeny block · keywordAllow when no allowlist · **critical break** toggle → Settings escape when `breakCritical`); CC columns 2|3; Beacon blurb + Clear recents; **Dock & menu bar** layout/size/rounding/autohide Facts → shell mtime reload |
...
| Sound → Output / Input / Applications / Mixer / Latency (`sound`) | `shipped` — iced hub + leaves; **Mixer** Wave Link–style grid thin (channels/inputs × mixes · route · volume · Ensure In · **row peaks** live tick · **channel reorder ↑↓** + **free-pointer grip drag** via `audio-mix.py move-channel`); Speakers/mix listen + rename when dump provides them; honest setup CTA; graph editor escape (`qpwgraph` Install… → Repos); resident `proteus-audio-mix serve` (Python mutations + fallback); Output/Input/Apps/Latency; pactl + `pw-metadata` |
...
| Privacy & security (transparency · mute · session · grants) | `partial` — iced hub: **In use now**; category Allow/Ask/Deny; **per-app grants thin** (permissions.json `apps` ∪ desktop apps · `store-set-app`); **Flatpak** mic/camera overrides (`proteus-permissions.py`); **Diagnostics** readiness (permissions.json · portal-sync · activity). **Shell Ask** Allow-once (`session-allow` + dual-shape session JSON) / Deny (`store-set-app` + enforce); **launch→Ask producer** (Beacon/Dock defer when mic/camera/screen grant=`ask` · resume on Allow); **capture enforce** on Deny + thin poll while privacy dots lit; `chrome privacyAsk <cat> [app] [pending]`. Out: AppArmor · mid-session attribution polish · catalog-driven perm lists beyond capture cats |
