...
| Privacy & security (transparency · mute · session · grants) | `partial` — iced hub: **In use now**; category Allow/Ask/Deny; **per-app grants thin** (permissions.json `apps` ∪ desktop apps · `store-set-app`); **Flatpak** mic/camera overrides (`proteus-permissions.py`); **Diagnostics** readiness (permissions.json · portal-sync · activity). **Shell Ask** Allow-once (`session-allow` + dual-shape session JSON) / Deny (`store-set-app` + enforce); **capture enforce** on Deny + thin poll while privacy dots lit; `chrome privacyAsk <cat> [app]`. Out: AppArmor · launch-gated Ask producer · mid-session attribution polish |
