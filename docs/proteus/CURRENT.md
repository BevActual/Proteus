...
| Online accounts (provider seats) | `partial` — **hub → per-provider leaves** (`AccountsPane` Connected / Add account · `AccountsProviderLeaf` · SettingsNav `accounts` hub); canonical provider list (Google / Microsoft / **Exchange** / Nextcloud / IMAP / CalDAV / CardDAV / Apple) with friendly blurbs — status/seats merge from `proteus-accounts` (stale catalog cannot inject Coming-later rows); OAuth Connect inline when client id ready; multi-seat Disconnect + OAuth Reconnect; vault tokens (not settings.json); **calendar + mail + contacts glances** (CalendarPanel · `CalendarEvents` / `MailGlance` / `ContactsGlance` · fetch scripts · IMAP/CalDAV/CardDAV/Apple/Exchange Graph — reconnect older Google/MS seats for write/send scopes); **calendar event create/edit/delete** (`proteus-calendar-mutate.py` · CalDAV + Google `calendar.events` + MS/Exchange `Calendars.ReadWrite` · CalendarPanel Add/Edit/✕ · title/day thin); **recurrence thin create** (daily/weekly/monthly · no end · create only); **mail compose thin** (`proteus-mail-send.py` · Google `gmail.send` + MS/Exchange `Mail.Send` + IMAP/Apple SMTP · To/Subject/Body); Proteus mail/contacts/Drive apps · CC/BCC/attachments/HTML/drafts/reply · series edit/COUNT/UNTIL/attendees · EWS/NTLM · Sign in with Apple OAuth Out |
