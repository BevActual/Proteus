...
+ row peaks live while Mixer open In; channel reorder ↑↓ In) · Notifications · Users · Privacy (activity +
...
Mixer free-pointer drag (↑↓ In), Tailscale deep ACL, Headscale ACL visual graph.
([STACK.md](./STACK.md) §6).
...
| Sound → Output / Input / Applications / Mixer / Latency (`sound`) | `shipped` — iced hub + leaves; **Mixer** Wave Link–style grid thin (channels/inputs × mixes · route · volume · Ensure In · **row peaks** live tick · **channel reorder ↑↓** via `audio-mix.py move-channel`); Speakers/mix listen + rename when dump provides them; honest setup CTA; graph editor escape (`qpwgraph` Install… → Repos); resident `proteus-audio-mix serve` (Python mutations + fallback); Output/Input/Apps/Latency; pactl + `pw-metadata`. **Out:** free-pointer drag |
