...
| **Beacon** — system search (`Super+Space` / `Super+D`; `Beacon.qml`, internal ids stay `launcher*`) | `partial` — **universal Apps surface**: one query answers apps + Settings + allowlisted Actions (non-`settings-*`) + calc, with a pinned **Search Files for “q”** escape; Apps / Files / Clipboard / Actions (Tab cycles · Ctrl+1–4; active mode pill labels); Esc clears query then closes; footer key legend; empty Apps = calm **Recents** hierarchy (or honest empty); empty Files = **Recents** + **Places** (or honest empty); Files search = Folders then Files (depth ≤5 · 40-cap · capped hint); `launcherFileRecents` on open; fuzzy + tags + Settings; EnvGate unavailable honesty (badge · reasons); calc/convert + near-miss hint; cliphist missing-vs-empty; allowlisted Actions (lock/logout/settings/CC/DND/Keep Awake/LocalSend/Wi‑Fi/Displays/Mixer/Privacy & security/Updates/power); `chrome beacon <q>` / `beaconState` + `lock unlock` IPC probes (dogfood/smoke) |
...
| Privacy & security (transparency · mute · session) | `partial` — what leaves (weather/accounts/probe/no telemetry); `weatherEnabled` mute; DND / Lock / clear clipboard / LocalSend jump; app permission categories still not enforced |
...
| Dual-path chrome (GUI + keyboard) | `shipped` — mouse-legible Settings IA; `Super+,`; Beacon Settings search; Actions (Wi‑Fi / Displays / Mixer / Privacy & security / Updates + session, inline in Apps mode); Settings `/` typeahead jump; hub lists ↑↓ Enter |
