...
| About (hardware class / capabilities) | `shipped` — OS/kernel/hostname (`SystemInfo`); Hyprland/Quickshell versions; tip hash; hw-probe class/caps; CPU/mem/uptime strip (`SystemLoad`, About-active only); Mission Center Activity Monitor escape; soft Hyprland profile picker; Copy system info + Copied; session actions only under Users (no About Session stub); hard posture switch Out; no in-Settings live dashboard |
