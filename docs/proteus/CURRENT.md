...
| App launcher (`Super+Space` / `Super+D`) | `partial` — Apps / Files / Clipboard / Actions modes (Ctrl+1–4); fuzzy + tags + Settings; calc/convert; cliphist clipboard; home-folder file search; allowlisted Actions (lock/logout/settings/CC/DND/power) |
