...
| Network → This machine / Devices / Diagnostics / Wi‑Fi / Bluetooth / LocalSend / Tailscale / VPN / Headscale (`network`) | `shipped` — hub + leaves; password Wi‑Fi; BT pair; Devices IPv4; **Diagnostics** (iface bars · `ss` · firewall · route/DNS · ping · Wireshark); LocalSend; Tailscale peers/exit/login-server; VPN up/down + **WireGuard + OpenVPN import** (`.ovpn` · optional session user/pass · `networkmanager-openvpn`); **Headscale admin thin** (`proteus-headscale.py` · vault API key · node list · expire/enable · browser escape); OpenVPN cert wizard · ACL/users/preauth/server install Out |
...
| Online accounts (provider seats) | `partial` — **hub → per-provider leaves** (`AccountsPane` Connected / Add account · `AccountsProviderLeaf` · SettingsNav `accounts` hub); canonical provider list (Google / Microsoft / **Exchange** / Nextcloud / IMAP / CalDAV / CardDAV / Apple) with friendly blurbs — status/seats merge from `proteus-accounts` (stale catalog cannot inject Coming-later rows); OAuth Connect inline when client id ready; multi-seat Disconnect + OAuth Reconnect; vault tokens (not settings.json); **calendar + mail + contacts glances** (CalendarPanel · `CalendarEvents` / `MailGlance` / `ContactsGlance` · fetch scripts · IMAP/CalDAV/CardDAV/Apple/Exchange Graph — reconnect older Google/MS seats for write/send scopes); **calendar event create/edit/delete** (`proteus-calendar-mutate.py` · CalDAV + Google `calendar.events` + MS/Exchange `Calendars.ReadWrite` · CalendarPanel Add/Edit/✕ · title/day thin); **recurrence thin create+edit** (daily/weekly/monthly · no end · whole series); **mail compose thin** (`proteus-mail-send.py` · Google `gmail.send` + MS/Exchange `Mail.Send` + IMAP/Apple SMTP · To/Subject/Body); Proteus mail/contacts/Drive apps · CC/BCC/attachments/HTML/drafts/reply · this-vs-all/COUNT/UNTIL/attendees/exceptions · EWS/NTLM · Sign in with Apple OAuth Out |
| Privacy & security (transparency · mute · session · grants) | `partial` — hub + leaves; what leaves; weather mute; DND / Lock / clipboard / LocalSend; **In use now** (mic/camera/screen apps via `privacy-indicators.py`); category Allow/Deny + per-app Allow/Ask/Deny in `permissions.json` (`proteus-permissions.py` · `Permissions.qml`); Flatpak mic/camera overrides; **portal PermissionStore sync** (devices mic/camera + screencast best-effort); **capture enforce** (Deny + Ask mute/destroy active mic/camera/**screen** screencast-like PW nodes; session Allow-once via runtime session file); **Ask prompt** (`PrivacyAsk` · Allow once / Always Allow / Deny at Dock/Beacon launch **+ mid-session mic/camera/screen** via activity probe); EnvGate ask≠deny; **fail-closed until `Permissions.ready`** (privacy-gated apps + Diagnostics); smoke/install harness; **not a full OS sandbox** (no AppArmor/v4l2 ACL · portal session Close / perfect screencast attribution Out) |
