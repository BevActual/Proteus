...
| Control Center (notifications + quick settings) | `shipped` — list empty/Dismiss honesty; toast/`showToast` SoT (DND · CC-open suppress); **unified Sound plate** (master on plate · Listen ▾ with optimistic switch · Sources ▾ On/Off chips + peaks · Mixer ›); elevated tiles; **Keep Awake** · **LocalSend** · **Power** profile menus; footer deep Sound/Network/Power → Settings; Settings/NM escapes; no Settings Notifications pane |
...
| App launcher (`Super+Space` / `Super+D`) | `partial` — Apps / Files / Clipboard / Actions (Ctrl+1–4; active mode pill labels); empty Apps = calm **Recents** hierarchy (or honest empty); empty Files = **Recents** + **Places** (or honest empty); Files search = Folders then Files (depth ≤5 · 40-cap · capped hint); `launcherFileRecents` on open; fuzzy + tags + Settings; EnvGate unavailable honesty (badge · reasons); calc/convert + near-miss hint; cliphist missing-vs-empty; allowlisted Actions (lock/logout/settings/CC/DND/Keep Awake/LocalSend/Wi‑Fi/Displays/Mixer/Privacy/Updates/power) |
...
| Dual-path chrome (GUI + keyboard) | `shipped` — mouse-legible Settings IA; `Super+,`; Spotlight Settings search; Actions (Wi‑Fi / Displays / Mixer / Privacy / Updates + session); Settings `/` typeahead jump; hub lists ↑↓ Enter |
