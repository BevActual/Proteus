...
| Feature | Status |
|---------|--------|
| Top bar (workspaces, title, clock+weather center, CC icon) | `partial` — wallpaper-first glass menu bar (`menuBarAlpha` clearer than dock; soft text outline when thin); **left**: app name (desktop-entry lookup via `ActiveWindow.barText`) + dynamic workspace strip (pills grow to the highest live workspace, min 4 / cap 10; occupied dots; wheel cycles; "+" jumps to next); **center**: date · time · temperature cluster (`ddd MMM d h:mm AP · 22°C`; weather chip only when `Weather.ready`) → click opens the **calendar popover** (`CalendarPanel`: today header, locale-aware month grid with today disc, ‹ · › month nav, weather summary H/L + location with honest no-location state; `chrome calendar` IPC); **right**: status cluster (unread badge · DND · Awake · battery) + **Control Center glyph** (three mini sliders with staggered knobs — ThemeSlider language; knobs slide + accent while open); Beacon button removed from the bar (dock pin + `Super+Space` own it); battery % only with a real battery (`Power.hasBattery` — VM/desktop shows none, not 0%) |
| Control Center (notifications + quick settings) | `shipped` — panel on chrome glass (`menuPlateFill`, blur layerrule) with slide+fade+scale open/close motion (layer window stays mapped for the exit; click-through while closing); notification cards with app-icon chip, relative "2m" timestamps (`Notifications.receivedAt`), slide-out dismiss; **Now Playing** MPRIS plate (art · title/artist · prev/play/next; hidden without a player); toast/`showToast` SoT (DND · CC-open suppress); **unified Sound plate** (master on plate · Listen ▾ · Sources ▾ chips + peaks · Mixer ›); **Display plate** (brightness slider; honest-skip without backlight); icon-disc tiles (accent disc = active) with press scale + animated popups; **Wi‑Fi radio toggle** menu on the Network tile (nmcli; wired-only hosts jump to Settings → Network); **Bluetooth power tile** (bluetoothctl; hidden without an adapter — Network spans the row); Power tile battery-honest; **Keep Awake** · **LocalSend** · **Power** menus; **system glance strip** (`SystemLoad.watching` only while open → Mission Center escape / Install); footer **Lock · Screenshot (grim + toast) · Settings**; `chrome` IPC probe; no Settings Notifications pane |
...
| **Beacon** — system search (`Super+Space` / `Super+D`; `Beacon.qml`, internal ids stay `launcher*`) | `partial` — **universal Apps surface**: one query answers apps + Settings + allowlisted Actions (non-`settings-*`) + calc, with a pinned **Search Files for “q”** escape; Apps / Files / Clipboard / Actions (Tab cycles · Ctrl+1–4; active mode pill labels); Esc clears query then closes; footer key legend; empty Apps = calm **Recents** hierarchy (or honest empty); empty Files = **Recents** + **Places** (or honest empty); Files search = Folders then Files (depth ≤5 · 40-cap · capped hint); `launcherFileRecents` on open; fuzzy + tags + Settings; EnvGate unavailable honesty (badge · reasons); calc/convert + near-miss hint; cliphist missing-vs-empty; allowlisted Actions (lock/logout/settings/CC/DND/Keep Awake/LocalSend/Wi‑Fi/Displays/Mixer/Privacy/Updates/power); `chrome beacon <q>` / `beaconState` + `lock unlock` IPC probes (dogfood/smoke) |
| Dock (pins, magnify, running dots) | `shipped` — continuous frosted glass shelf (`glassAlpha` frost floor + curve-following edge glow; no straight specular); smooth magnify; running disc vs active accent pill; hairline divider pins ‖ transients; launch bounce until first window; click cycles app windows when focused; long-press edit (−/+ · Done); press-drag reorder / drag-off remove; glass Keep/Remove/Quit (`ChromeMenuPlate`) |
| Session start (`proteus-session`) | `shipped` — prefers `start-hyprland` (known paths; fail-closed to Hyprland); hypr seed `exec-once` = qs/bg/cliphist/polkit agent (install strips terminal autostart); Settings float+center windowrule (0.53+ `match:` syntax); `hide-system-apps` via apps + post-install (Settings-covered tools + Quickshell; Calculator stays); host `session-smoke` + `install-smoke` |
| Desktop widgets (free place; Customize) | `shipped` — long-press (empty desktop or a widget) or `Super+Shift+W` (probe: `chrome customizeDesktop` IPC); free-place + optional Snap to Grid (center graph · no overlap); **tight packing** (frame width/height tables match the drawn cards — no invisible slack; `overlapGap: 0` lets edges touch, never overlap); **alignment guides** while free-dragging (accent hairlines magnetize edges/centers to neighbors + surface center, 6px threshold; dropped if collision resolution moves the frame); **arrow keys nudge the selected widget** (8px · Shift 40px · grid cell when snapping; Customize grabs the keyboard so Esc/arrows land); **widgets are click-interactive outside Customize** (clock/calendar → calendar popover · weather → popover or Settings → Date & time when no location · system → Mission Center/Software · battery → Settings → Power · note → edit in place · world clock → city picker) — the old per-widget hold timer armed on an unaccepted press and fired customize after normal clicks (phantom Customize); long-press now lives on the surface (with press hit-test select) + interactive areas' own `onPressAndHold`; **Add Widget gallery renders live scaled previews** (real widget instances, non-interactive; catalog glyph while loading); catalog via `Widgets.qml` — clock · media · battery · weather · **calendar** (today tile at S, month grid + today disc at M/L, midnight rollover) · **system glance** (CPU/mem bars + uptime via `SystemLoad` retain/release refcount; storage at L) · **note** (sticky — click to write in place, debounced save to `noteText`, widget layer raises + grabs keyboard while editing via `ShellState.desktopNoteEditing`; read-only on lock) · **world clock** (first multi-instance type, `unique: false` — one per city; `TZ=<zone> date` owns the tz math; in-widget city picker persists `tzId`/`tzLabel`); separate from lock; **not** in Settings; widget store Out |
...
| Users (session actions + read-only local users · greetd status) | `shipped` — Session Lock/Logout; Reboot/Shutdown confirm strip; current user GECOS/home/UID/groups; other users read-only; Online accounts jump; greetd status + conf escape; no add/remove; Settings does not write greeter prefs |
...
| Privacy (transparency · mute · session) | `partial` — what leaves (weather/accounts/probe/no telemetry); `weatherEnabled` mute; DND / Lock / clear clipboard / LocalSend jump; app permission categories still not enforced |
...
| Window & layout | `shipped` — floating sheet 820×560 (hypr `windowrule float`+`center`; seed + config.sh append); wide/tiled windows cap + center the pane column (`paneMaxW` 760; back/title track it, ✕ stays at the corner; Mixer full-bleed) |
| Dual-path chrome (GUI + keyboard) | `shipped` — mouse-legible Settings IA; `Super+,`; Beacon Settings search; Actions (Wi‑Fi / Displays / Mixer / Privacy / Updates + session, inline in Apps mode); Settings `/` typeahead jump; hub lists ↑↓ Enter |
...
| `./scripts/smoke/install-smoke.sh` | Overlay installer tree check |
...
| `./scripts/smoke/qs-guest-smoke.sh` | Guest cold-start `SHELL_OK` / `SETTINGS_OK` + record `quickshell` version + polkit agent + nav deep link + Beacon universal search probe |
