...
| Displays scale / mode / orientation / layout | `proteus-compositorctl` `output` scale/pos/mode/transform + `~/.config/proteus/displays.json`; Identify flash; 10s full-snapshot Revert (Settings); compositor/Fact + Settings orientation UI **In** (Normal/90/180/270 · Flipped / Flip 90/180/270 · wl 0–7) |
...
| App mixer (Wave Link–style) | Channels (default Apps/…) + mic/line inputs × mixes; Speakers/mix listen; rename; route/volume/Ensure In; **row peaks** live while open; **channel reorder ↑↓** + **free-pointer grip drag** (`move-channel`); × confirm; graph escape (`qpwgraph`, Install… → Repos); `~/.config/proteus/audio-mix.json`; resident `proteus-audio-mix serve`; mutations `audio-mix.py`; **CC Sound plate** |
...
| **Displays** (`displays`) | Layout canvas + per-monitor scale/orientation; Identify; 10s Revert; Refresh/topology honesty; Fact `displays.json` (incl. transform) | compositorctl + `displays.json` | `shipped` (thin; orientation + flipped transforms In) |
...
| **Privacy & security** (`privacy`) | Hub → What leaves + weather mute + session; **In use now**; category leaves (Allow/Ask/Deny + **per-app grants thin** via `store-set-app`; category Ask CLI In); Flatpak overrides; portal PermissionStore sync; capture enforce (Deny + Ask mute/destroy mic/camera/**screen**); **portal Session.Close** best-effort; **Shell Ask** Allow-once / Deny + **Beacon/Dock launch→Ask** (`session-allow` · dual-shape session JSON · `chrome privacyAsk`). Out: AppArmor · mid-session attribution polish | `permissions.json` · `proteus-permissions.py` · PrivacyAsk · PrivacyIndicators · shell-core gate · portal Session.Close + pactl/PW enforce | `partial` |
...
| Keep Awake | **Shell Control Center** duration menu (+ menu-bar **Awake** when on; Beacon Actions) — temporary `systemd-inhibit idle:sleep` so owned idle / logind skip idle lock & sleep; compositor also holds `--what=idle` while `zwp_idle_inhibit` clients inhibit; **not** a Settings Power control |
...
| Focus | Soft quiet profiles (seed Work/Personal + **add/rename/delete** In thin); keywordAllow/Deny CSV In; **allowedApps** CSV In; **schedule** `{enabled, days[1–7], start, end, rrule?}` In + shell edge auto-apply; thin **RRULE** `FREQ=WEEKLY;BYDAY=` (+ optional `UNTIL`) In; **shell Beacon/Dock launch enforce** when Focus on (allowedApps whitelist · keywordDeny · keywordAllow); **critical break** (`breakCritical` · Settings escape) In; active chips + enable; CC menu + Desktop → Focus leaf |
...
| Beacon | Universal search: **mode strip** Apps · Settings · Windows · Files (**Ctrl+1–4**) · Files index (`beacon-file-index`) · **Clipboard** + **Calc** in Apps · tags / unit convert Out; clear recents In |
...
transforms **In** (wl 0–7). Not a leaf-split hub.

| Concern | Role |
|---------|------|
| Layout canvas | Drag + edge snap; Apply layout |
| Scale / orientation / Identify | Scale presets; Normal/90/180/270 · Flip / Flip 90/180/270; Identify → `dispatch identify` |
...
| Mixer | Wave Link–style grid thin: channels/inputs × mixes; Speakers vs mix listen; Level; rename; expand/listen; route · volume · Ensure In · **row peaks live while open** · **channel reorder ↑↓** + **free-pointer grip drag**; add channel/input/mix when dump supports; graph editor escape (`qpwgraph`, Install… → Repos). Quick per-source adjust also in Control Center Sources ▾ |
...
| Headscale | Remote admin URL + vault API key; node list; expire/enable; users list/create; policy HuJSON check/save (db mode) + **structured ACL groups thin** (JSON-compatible subset · member add/remove rewrites draft); Open admin UI; does not run Headscale locally. **Out:** visual ACL graph · full HuJSON comment round-trip |

| Pane | Live apply | On-disk / helper |
|------|------------|------------------|
| Network | `hostnamectl` · `nmcli` wifi/VPN up·down · WG/OpenVPN `connection import` · `bluetoothctl` · `tailscale` up/down/`--login-server=`/`--exit-node=` · `proteus-headscale.py` · clipboard IP | Escape: blueman / NetworkManager / Wireshark / browser admin — cert path / user-pass · OpenVPN PKI/PKCS#11 · Tailscale deep ACL · Headscale preauth / ACL visual graph · in-pane capture Out |
...
glances thin** — list every seat; create/edit/disconnect upsert per seat without
clobbering siblings) + Google/Microsoft/Exchange PKCE + Nextcloud
...
4. ~~**Network polish**~~ — largely shipped (IPv4 on Devices · Diagnostics ss/firewall · WG/OpenVPN import thin · Headscale admin thin + users/policy text + structured ACL groups thin); cert path / user-pass · OpenVPN PKI/PKCS#11 · Headscale preauth / ACL visual graph stay Out  
...
*(Sound Mixer honesty: grid route/volume/Ensure + row peaks live + channel reorder ↑↓ + free-pointer grip drag In.)*
...
Super+1–10 logical SoT + Scratchpad ◇ · Super+S/Alt+S + custom special CRUD
...
settings.json `focusProfiles` — shell schedule auto-apply + launch enforce In;
critical break UI In · thin WEEKLY RRULE In · duplicate/reorder · CC inline CRUD Out.)*
