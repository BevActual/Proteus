...
| App mixer (Wave Link–style) | Channels (default Apps/…) + mic/line inputs × mixes; Speakers/mix listen; rename; route/volume/Ensure In; **row peaks** live while open; **channel reorder ↑↓** (`move-channel`); × confirm; graph escape (`qpwgraph`, Install… → Repos); `~/.config/proteus/audio-mix.json`; resident `proteus-audio-mix serve`; mutations `audio-mix.py`; **CC Sound plate**. **Out:** free-pointer drag |
...
| Mixer | Wave Link–style grid thin: channels/inputs × mixes; Speakers vs mix listen; Level; rename; expand/listen; route · volume · Ensure In · **row peaks live while open** · **channel reorder ↑↓**; add channel/input/mix when dump supports; graph editor escape (`qpwgraph`, Install… → Repos). Quick per-source adjust also in Control Center Sources ▾. **Out:** free-pointer drag |
...
*(Sound Mixer honesty: grid route/volume/Ensure + row peaks live + channel reorder ↑↓ In — free-pointer drag Out.)*
