...
cd ~/Projects/Proteus
PROTEUS_ROOT="$PWD" sudo -E bash install/bootstrap.sh
```
...
| 2 | `~/.config/proteus/root` | `install/config.sh` |
...
Package sets: [`proteus-base.packages`](../../install/proteus-base.packages)
(session stack) · [`proteus-desktop.packages`](../../install/proteus-desktop.packages)
(browser, files, viewers, capture) · [`proteus-console.packages`](../../install/proteus-console.packages)
...
- **Lock unlock PIN** — `apps` stage puts `proteus-pin.py` + `check-unlock.py`
  ...
  `shell/pam/proteus-lock` via `install/machine/install-lock-pam.sh`; if that step
