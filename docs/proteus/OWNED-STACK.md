...
| **2. Compositor** | Smithay `compositor/` — **Hyprland purged**; DRM session + Displays Fact + keybinds + nested winit; shell IPC `wm_ipc.rs`; `proteus-settings-apply`; owned idle | Deeper multi-GPU / transform still out — [COMPOSITOR-SPIKE.md](./COMPOSITOR-SPIKE.md) |
