...
| **2. Compositor** | Smithay `compositor/` — **Hyprland purged**; DRM session + Displays Fact (incl. live transform) + keybinds + nested winit; shell IPC `wm_ipc.rs`; `proteus-settings-apply`; owned idle; protocol lock opt-in dogfood | Deeper multi-GPU / Settings transform UI / blur still out — [COMPOSITOR-SPIKE.md](./COMPOSITOR-SPIKE.md) |
